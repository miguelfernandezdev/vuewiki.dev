...
Por defecto, `ref()` y `reactive()` hacen reactiva cada propiedad anidada. Para un objeto de configuración con cientos de claves, o una estructura de datos de solo lectura que recibes de una API y nunca mutas en profundidad, esto es un desperdicio porque Vue recorre el objeto completo al inicializarlo para adjuntar el seguimiento. `shallowRef` y `shallowReactive` hacen reactivo solo el nivel superior, lo cual es suficiente cuando reemplazas el objeto entero en lugar de mutar propiedades anidadas.
